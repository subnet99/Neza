...
from neza.base.utils.weight_utils import (
    process_weights_for_netuid,
    convert_weights_and_uids_for_emit,
)
from neza.utils.config import add_validator_args
import traceback
...
class BaseValidatorNeuron(BaseNeuron):
    ...
    def __init__(self, config=None):
        ...
        # Register base block callbacks
        self.register_block_callback(self.log_on_block)
        self.register_block_callback(self.set_weights_on_interval)

    def log_on_block(self, block):
        """
        Basic implementation of logging information on each block
        Subclasses can override this method to provide custom logging

        Args:
            block: Current block number
        """
        # Calculate blocks until next weight setting
        blocks_till = self.config.neuron.epoch_length - (
            block % self.config.neuron.epoch_length
        )

        # Log detailed information every 10 blocks
        if block % 10 == 0:
            bt.logging.info(f"Block: {block} | Blocks till Set Weights: {blocks_till}")

    def set_weights_on_interval(self, block):
        """
        Basic implementation of setting weights on each epoch
        ...
        try:
            # Call set_weights method to set weights
            self.set_weights()
    ...
    def run(self):
        ...
        # Check that validator is registered on the network using a separate thread
        # to avoid WebSocket concurrency issues
        ...
        try:
            while True:
                ...
                # Sync metagraph and potentially set weights in a separate thread
                # to avoid WebSocket concurrency issues
                self._sync_in_thread()
    ...
    def _sync_in_thread(self):
        """
        Run sync operation in a separate thread to avoid WebSocket concurrency issues
        """
        sync_thread = threading.Thread(target=self.sync)
        sync_thread.daemon = True
        sync_thread.start()
        sync_thread.join()  # Wait for sync to complete
    ...
    def set_weights(self):
        ...
        (
            ...
            websocket_lock=self._subtensor_lock,
        )
